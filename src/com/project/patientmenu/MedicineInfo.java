...
/***
 * 
 * MedicineInfo 클래스입니다. 환자가 처방 받은 약을 조회합니다.
 * @author 2조
 *
 */
public class MedicineInfo { //처방 받은 약 정보 확인

	/***
	 * 메소드 입니다. 환자의 번호를 입력받아 환자가 처방받은 약을 조회합니다.
	 * @param pNumber 환자 번호
	 */
	public static void begin(int pNumber) {
		...
		while(flag) {
			...
			System.out.printf("    [[%s님 처방약 기록(환자용)]]   \n", pname);
			System.out.println("===================================================");
			
			
			System.out.println("[날짜]\t\t[처방받은 약]\t\t[담당의사]");
					...
					for(MedicalInfo m1 : Data.medicalInfoList) {
						if(m1.getmNumber() == pNumber) {
							if(p.getpConnectionpNumber() == d.getdConnectionNumber()) {
								System.out.printf("%s\t%s\t        %s\n", m1.getmDate(), m1.getmMedicine(), d.getdName());
							} //if
